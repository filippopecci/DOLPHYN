function ng_to_power!(EP,inputs,setup)
    ...
    @variable(EP, vNGP[y in Pgen, t in PowerDays] >=0)
    ...
    @constraint(EP,cNgGas2Power[y in Pgen,t in PowerDays], vNGP[y,t] ==  (dfGen[y,:Heat_Rate_MMBTU_per_MWh]*scale_factor)*sum(EP[:vP][y,s] for s in (h(t)-1)*24 + 1 : h(t)*24))
