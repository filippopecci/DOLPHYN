...
@doc raw"""
	co2_cap_all_sectors(EP::Model, inputs::Dict, setup::Dict)
...
"""
function co2_cap_all_sectors(EP::Model, inputs::Dict, setup::Dict)

	
	if setup["SystemCO2Constraint"] ==1 # Independent constraints for each sector
		
		if setup["CO2Cap"]>=1
			# CO2 constraint for power sector imposed separately
			co2_cap!(EP, inputs, setup)
		end
		
		if setup["ModelH2"]==1 && setup["H2CO2Cap"]>=1
			# CO2 constraint for H2 sector imposed separately
			co2_cap_hsc!(EP,inputs,setup)
		end

		if setup["ModelNG"]==1 && setup["NGCO2Cap"]>=1
			# CO2 constraint for NG sector imposed separately
			co2_cap_ng!(EP,inputs,setup)
		end

	elseif setup["SystemCO2Constraint"] ==2 # Joint emissions constraint
		...
		print_and_log("C02 Policies Module for all sectors combined")
		...
		elseif setup["CO2Cap"] == 2 
			if setup["ModelNG"]==1 && setup["NGCO2Cap"]>=1
				...
				# MaxCO2Rate is kton/MWH, so need to adjust H2 demand to be in ktonne as well  on RHS of constraint if ParameterScale=1
				@constraint(EP, cCO2Emissions_systemwide[cap=1:inputs["NCO2Cap"]],
					sum(inputs["omega"][t] * EP[:eEmissionsByZone][z,t] for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]), t=1:T) +
					sum(inputs["omega"][t] * EP[:eH2EmissionsByZone][z,t] for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]), t=1:T) <=
					...
					sum(inputs["dfMaxCO2Rate"][z,cap] * H2_LHV/scale_factor * sum(inputs["omega"][t] * (inputs["H2_D"][t,z] + EP[:eH2DemandByZoneG2P][z,t] - sum(EP[:vH2NSE][s,t,z] for s in 1:H2_SEG)) for t=1:T) for z = findall(x->x==1, inputs["dfCO2CapZones"][:,cap]))
				)
			end
		...
		elseif (setup["CO2Cap"]==3)
			if setup["ModelNG"]==1 && setup["NGCO2Cap"]>=1
				...
				# MaxCO2Rate is kton/GWH, so need to adjust H2 demand to be in ktonne as well  on RHS of constraint if ParameterScale=1
				@constraint(EP, cCO2Emissions_systemwide[cap=1:inputs["NCO2Cap"]],
					sum(inputs["omega"][t] * EP[:eEmissionsByZone][z,t] for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]), t=1:T) +
					sum(inputs["omega"][t] * EP[:eH2EmissionsByZone][z,t] for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]), t=1:T) <=
					sum(inputs["dfMaxCO2Rate"][z,cap] * inputs["omega"][t] * EP[:eGenerationByZone][z,t] for t=1:T, z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]))+
					sum(inputs["dfMaxCO2Rate"][z,cap] *H2_LHV/scale_factor *inputs["omega"][t] * EP[:eH2GenerationByZone][z,t] for t=1:T, z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]))
				)
			end
		end 
